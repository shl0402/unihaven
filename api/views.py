...
class AccommodationListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if location_data:
            # Make sure universities field is present in validated_data, default to current university
            if 'universities' not in serializer.validated_data:
                # Get University object for current university
                from .models import University
                try:
                    university = University.objects.get(code=self.request.university)
                    # We can't modify validated_data directly in this context
                    # Instead we'll handle it when saving
                except University.DoesNotExist:
                    raise serializers.ValidationError({
                        'universities': f"University with code '{self.request.university}' not found"
                    })
                
            # Save with location data from geocoding
            ...
            # If universities were not provided, add the current university
            if 'universities' not in serializer.validated_data:
            ...
            for campus in campuses:
                ...
                # Use update_or_create instead of create to avoid unique constraint violations
...
@extend_schema(
    ...
    summary="Search accommodations (don't use get accommodation, use this instead)",
    description="""
